...
When bundling `mitek-science-sdk.js` with webpack, you may see:

- `Critical dependency: the request of a dependency is an expression`

The SDK includes dependencies that use dynamic `require()` for Node-only paths. Webpack cannot statically resolve these. In the browser that code path never runs, so the warning is safe to suppress.
...
### Bundler errors on named imports from mitek-science-sdk.js

**Symptoms:** Build fails with errors like `"cmd" is not exported by "mitek-science-sdk.js"`.

**Cause:** The SDK is a UMD bundle, not an ES module with static exports.

**Fix:** Use `import "./mitek-science-sdk.js"` and call `mitekScienceSDK.cmd(...)`. Use `import type { ... } from "./mitek-science-sdk.js"` for TypeScript types. See [Installation — Option B](../INSTALLATION.md#option-b-bundler-import).
...
- Alternatively, install SDK v5.9.0+. These newer versions use a WASM instead of WebGL.

**AI-based RTS errors:**
- AI-based RTS uses anti-debugging techniques that block DevTools when using the **production** (`-prod`) SDK package. Use the **development** (`-dev`) package for local development. Do **not** deploy the **`-dev`** package to production. See [SDK build variants](../INSTALLATION.md#sdk-build-variants).
