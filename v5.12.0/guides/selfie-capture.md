...
| `MITEK_ERROR_FOCUS`     | "Focusing" (see Note)                |

>**Note**: As of 5.12.0, `MITEK_ERROR_FOCUS` is no longer emitted for selfie auto capture.
...
Optional RTS-related options use the same `options` object. For full details, see [Capture options — AI-Based RTS](../configuration/capture-options.md#ai-based-rts-real-time-security).


| Option                     | Summary                                                               |
| -------------------------- | --------------------------------------------------------------------- |
| `aiBasedRtsPayloadSize`    | `"small"` (default) or `"normal"`                                     |
| `aiBasedRtsEncryptionKey`  | `publicKey` (valid base64) and optional `keyId`; omit for SDK default |
| `aiBasedRtsAdditionalData` | Optional string metadata for AI-based RTS (e.g. `JSON.stringify({ sessionId: "…" })` if your integration expects JSON) |
