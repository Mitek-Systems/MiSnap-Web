...
| `VOICE` | Voice recording | Voice biometrics (via `CAPTURE_AND_PROCESS_VOICE`) |
...
mitekScienceSDK.cmd("CAPTURE_AND_PROCESS_FRAME", {
  ...
  options: {
    ...
    aiBasedRtsEnabled: true        // Enable AI-based RTS
  }
});
```
...
Captures voice for biometrics. Uses a different command:
